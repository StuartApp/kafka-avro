 ...
 * @private
 */
Producer.prototype._serializeType = function (topicName, isKey, key, value) {
  var schema = this.sr.keySchemas[key];
  var schemaType = 'key';
  ...
  if (!schema) {
    ...
    var schemaId = this.sr.schemaIds[key + '-' + schemaType];

    return this.serialize(schema, schemaId, value);
  }
};
...
Producer.prototype._produceWrapper = function (producerInstance, topicName,
  ...
  var sendValue = this._serializeType(topicName, true, key, value)

  return producerInstance.__kafkaAvro_produce(topicName, partition, sendValue,
    null, timestamp, optOpaque);
};
